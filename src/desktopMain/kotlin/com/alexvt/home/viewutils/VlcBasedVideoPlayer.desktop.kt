...
// see https://github.com/mahozad/compose-video-player/blob/vlc/src/main/kotlin/VideoPlayer.kt
...
private fun VideoPlayerImpl(
    ...
    val mediaPlayerComponent = initializeMediaPlayerComponent(onClick, onLongOrRightClick)
    ...
    LaunchedEffect(url) { mediaPlayer.media().start(url) }
...
private fun initializeMediaPlayerComponent(
    ...
    val mouseListener = object : MouseListener {
        override fun mouseClicked(event: MouseEvent?) {
            event?.button?.let { if (it == BUTTON1) onClick() else onLongOrRightClick() }
        }
    ...
    if (isMacOS()) {
        CallbackMediaPlayerComponent().apply {
            videoSurfaceComponent().addMouseListener(mouseListener)
        }
    } else {
